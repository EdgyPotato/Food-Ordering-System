...
<body class="bg-gray-200 flex flex-col items-center justify-center h-full">
    <div class="fixed flex flex-col items-center w-full top-0 pt-4 bg-gradient-to-r from-pigment-indigo-500 to-pigment-indigo-400 rounded-b-3xl shadow-xl">
        ...
        <div class="flex justify-between items-center w-full pt-6 pb-6 text-white font-bold text-center max-w-md">
            <div class="w-1/3"><a href="#food">Food</a></div>
            <div class="w-1/3"><a href="#beverage">Beverages</a></div>
            <div class="w-1/3"><a href="#other">Others</a></div>
        </div>
    </div>
    ...
    <form id="form" action='tofood' method='get' enctype='multipart/form-data' class="w-full h-full flex justify-center pb-20">
        <input type="hidden" id="hidden" name="id">
        <div class="pt-52 w-full flex justify-center max-w-md">
            <div class="flex flex-col items-center justify-start w-11/12 bg-grey-100 shadow-md rounded-xl pb-2">
                <?php if (count($menufood)) : ?>
                    <span class="w-full text-lg font-bold pl-4 pt-3 pb-1" id="food">Food</span>
                <?php endif; ?>
                <?php foreach ($menufood as $menus) { ?>
                    <div class="h-28 w-11/12 border-2 mb-2 p-1 pr-2 border-pigment-indigo-200 hover:bg-grey-200 rounded-xl bg-white" onclick='submitForm("<?php echo $menus->foodid ?>")'>
                        <div class="grid grid-cols-5 grid-rows-2 h-full">
                            ...
                            <div class="font-bold col-span-2 -ml-4 mt-1 pr-3"><?php echo $menus->foodid . '. ' . $menus->foodname ?></div>
                            <div class="font-bold -ml-3 mt-1 text-right"><?php echo 'RM ' . number_format($menus->price, 2) ?></div>
                            <div class="col-span-3 text-gray-500 text-sm -ml-4 -mt-5"><?php echo $menus->description ?></div>
                        </div>
                    </div>
                <?php } ?>
                <?php if (count($menubeverage)) : ?>
                    <span class="w-full text-lg font-bold pl-4 pt-3 pb-1" id="beverage">Beverages</span>
                <?php endif; ?>
                ...
                <?php if (count($menuother)) : ?>
                    <span class="w-full text-lg font-bold pl-4 pt-3 pb-1" id="other">Other</span>
                <?php endif; ?>
                <?php foreach ($menuother as $menus) { ?>
                    <div class="h-28 w-11/12 border-2 mb-2 p-1 pr-2 border-pigment-indigo-200 hover:bg-grey-200 rounded-xl bg-white" onclick='submitForm("<?php echo $menus->foodid ?>")'>
                        <div class="grid grid-cols-5 grid-rows-2 h-full">
                            ...
                            <div class="font-bold col-span-2 -ml-4 mt-1 pr-3"><?php echo $menus->foodid . '. ' . $menus->foodname ?></div>
                            <div class="font-bold -ml-3 mt-1 text-right"><?php echo 'RM ' . number_format($menus->price, 2) ?></div>
                            <div class="csol-span-3 text-gray-500 text-sm -ml-4 -mt-5"><?php echo $menus->description ?></div>
                        </div>
                    </div>
                <?php } ?>
            </div>
    ...
    <div class="fixed flex justify-between items-center bottom-0 w-full p-2 bg-pigment-indigo-200 rounded-t-2xl">
        ...
        <div class="px-2"><a href="addcart"><img src="<?php echo e(asset('image/cart.png')); ?>" width="40px" height="40px"></a></div>
    </div>
    <script>
...
</html><?php /**PATH C:\AD Project\AD-Project\resources\views/cusmenu.blade.php ENDPATH**/ ?>